...
const SKIP_DIRS = [".vite", "node_modules"];
...
function getHandlerFor(file) {
  ...
  if (name.endsWith(".module.css")) return stripCSSComments;
...
function stripJSComments(src) {
  ...
  out = out.replace(/\/\*[\s\S]*?\*\//g, "");
  // Remove single-line comments (//...) but preserve http:// and https:// and protocol-like strings
...
async function main() {
  try {
    ...
    if (!stat.isDirectory()) throw new Error("client/ not found");
  } catch (e) {
    console.error("client/ folder not found at", ROOT);
    process.exit(1);
  }
